...
def postprocess(output, img_w, img_h, input_shape, conf_th=0.8, nms_threshold=0.5, letter_box=True):
    ...
    print(output.shape)
    detections = output[0,:,0,0]
    num = detections[0]
    print(num)
    detections = detections[1:int(1+num*6)]
    ...
    if len(detections) == 0:
        ...
        detections[:, 0:4] *= np.array(
            [old_w/640, old_h/640, old_w/640, old_h/640], dtype=np.float32)
