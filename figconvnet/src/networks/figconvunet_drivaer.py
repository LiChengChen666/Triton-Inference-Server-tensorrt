...
class FIGConvUNetDrivAerNet(FIGConvUNet):
    ...
    def data_dict_to_input(self, data_dict) -> torch.Tensor:
        ...
        print(data_dict)
        c_d = data_dict['c_d'].float()[None,None,...]
        c_l = data_dict['c_l'].float()[None,None,...]
        c_lf = data_dict['c_lf'].float()[None,None,...]
        c_lr = data_dict['c_lr'].float()[None,None,...]
        f = torch.cat([c_d,c_l,c_lf,c_lr],dim=-1)
        f = f.repeat(1,vertices.shape[1],1)
        ...
        return vertices.to(self.device),f.to(self.device)

    @torch.no_grad()
    def eval_dict(self, data_dict, loss_fn=None, datamodule=None, **kwargs) -> Dict:
        vertices, c_d = self.data_dict_to_input(data_dict)
        normalized_pred, drag_pred = self(vertices,c_d)
        if loss_fn is None:
            loss_fn = self.loss
        normalized_gt = (
